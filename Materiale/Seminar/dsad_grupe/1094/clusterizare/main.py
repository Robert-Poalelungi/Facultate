...
def partitie(h, nr_clusteri, p, instante):
    """
    Extragem clusteri prin sectionarea dendrogramei in dreptul unui prag corespunzator lui nr_clusteri

    :param h: matricea de legaturi (linkage matrix)
    :param nr_clusteri: in cate clase vrem sa categorisim instantele
    :param p: numarul maxim de merge-uri/pasi pe care il putem efectua
    :param instante: observatii/tari/judete/indivizi, etc.
    :return:

    matricea de legaturi este o matrice care detaliaza intreg procesul de clusterizare. are n-1 randuri, unde n e
    numarul de observatii si fiecare rand e de forma:
    [Ci     Cj      var_dintre_Ci_Cj        size(Ci + Cj)]
    """

    # pozitia unde se afla pragul de sectionare al dendrogramei in matricea de legaturi
    ...
    # simluam procesul de clusterizare
    c = np.arange(n)
...
def execute():
    ...
    # partitionare cu un numar de clusteri definit apriori
    clusteri = [2, 3, 4, 5]
    ...
    print(f"Numarul optim de clusteri: {nr_clusteri}")

    partitie_optima = partitie(h, nr_clusteri, p, instante)

    # alternativa de extractie automata a clusterilor
    ...
    # pur facultativ, ca sa evidentiem diferentele intre metodele de clusterizare
