...
def partitie(h, nr_clusteri, p, instante):
    ...
    h = matricea de legaturi (linkage matrix) si ea contine tot procesul de clusterizare de la prima etapa, unde avem
        n clusteri (fiecare observatie reprezinta un cluster cu 1 element), pana la ultima etapa cand toate observatiile
        sunt grupate intr-un singur cluster

        [C1, C4, var_between_C1_C4, size(C1+C4)]
    """

    # positia sectionarii in linkage matrix (matricea de legaturi)
...
def execute():
    ...
    # standardizare (care e f importanta pentru metoda Ward)
    scaler = StandardScaler()
    x_std = scaler.fit_transform(x)

    # construim ierarhia de clusteri folosind metoda Ward
    h = hclust.linkage(x_std, method='ward')
    print(f'Linkage matrix: \n{h}')
    n = len(instante)
    p = n-1

    # generam si exportam partititii variate
    clusteri = [2,3,4,5]

    for k in clusteri:
        print(f"Paritite cu {k} clusteri")
    ...
    # alegerea nmarului optim de clusteri
